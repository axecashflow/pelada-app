...
import { groupManagers } from "@/app/infrastructure/db/schema";
...
describe("DrizzleGroupRepository (unit)", () => {
  ...
  describe("addManager", () => {
    it("should insert a row into group_managers", async () => {
      const repo = new DrizzleGroupRepository();
      ...
      await repo.addManager("group-1", "user-1");

      expect(db.insert).toHaveBeenCalledWith(groupManagers);
  ...
  describe("findGroupsByManagerId", () => {
    it("should return empty array if user is not a manager of any group", async () => {
      (db.select as jest.Mock).mockReturnValueOnce({
        from: jest.fn().mockReturnThis(),
        where: jest.fn().mockResolvedValueOnce([]),
      });
      ...
      const repo = new DrizzleGroupRepository();
      const result = await repo.findGroupsByManagerId("user-1");

      expect(result).toEqual([]);
    });

    it("should return list of groups managed by the user", async () => {
