...
export const countGoalsFromPlayer = (playerId: string, currentMatch: MatchViewModelType | null) => {
  if (!currentMatch) return 0;
  ...
  const goals = currentMatch.stats.filter(stat =>
    goalTypes.includes(stat.type) && stat.playerId === playerId
  ).length;

  return goals;
};

export const countAssistsFromPlayer = (playerId: string, currentMatch: MatchViewModelType | null) => {
  if (!currentMatch) return 0;
  ...
  const assists = currentMatch.stats.filter(stat =>
    assistTypes.includes(stat.type) && stat.playerId === playerId
  ).length;

  return assists;
};

export const countPreAssistsFromPlayer = (playerId: string, currentMatch: MatchViewModelType | null) => {
  if (!currentMatch) return 0;

  const preAssistTypes = [
    StatTypeEnum.PRE_ASSIST,
  ];

  const preAssists = currentMatch.stats.filter(stat =>
    preAssistTypes.includes(stat.type) && stat.playerId === playerId
  ).length;
...
export function calculatePlayerStats(matches: MatchViewModelType[]): PlayerStats[] {
  ...
  matches.forEach((match) => {
    [match.teamA, match.teamB].forEach((team) => {
      team.players.forEach((player) => {
        ...
        stats.totalRating += typeof player.rating === 'string' ? parseFloat(player.rating) : player.rating;
...
export function getTopScorer(playerStats: PlayerStats[]): PlayerStats | null {
  ...
  return filtered.reduce((prev, current) => current.goals > prev.goals ? current : prev);
}
...
export function getTopAssister(playerStats: PlayerStats[]): PlayerStats | null {
  ...
  if (filtered.length === 0) return null;
  return filtered.reduce((prev, current) => current.assists > prev.assists ? current : prev);
}
...
export function getTopDefender(playerStats: PlayerStats[]): PlayerStats | null {
  ...
  if (filtered.length === 0) return null;
  return filtered.reduce((prev, current) => current.tackles > prev.tackles ? current : prev);
}

export function getMostGoalParticipations(playerStats: PlayerStats[]): PlayerStats | null {
  if (playerStats.length === 0) return null;
  const filtered = playerStats.filter(p => p.goalParticipations > 0);
  if (filtered.length === 0) return null;
  return filtered.reduce((prev, current) => current.goalParticipations > prev.goalParticipations ? current : prev);
}

export function getTopDribbler(playerStats: PlayerStats[]): PlayerStats | null {
  if (playerStats.length === 0) return null;
  const filtered = playerStats.filter(p => p.dribbles > 0);
  if (filtered.length === 0) return null;
  return filtered.reduce((prev, current) => current.dribbles > prev.dribbles ? current : prev);
}

export function getBestAvgRating(playerStats: PlayerStats[]): PlayerStats | null {
  if (playerStats.length === 0) return null;
  return playerStats.reduce((prev, current) => current.avgRating > prev.avgRating ? current : prev);
}
