...
const ShotOnTarget = ({ currentMatch, recordMatchEvents }: ShotOnTargetProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: 'Erro',
        description: 'Selecione o autor da finalização no alvo.',
        variant: 'destructive',
      });
      return;
    }
    ...
    eventsToRecord.push({
      ...
      playerId: scorerId,
      statType: StatTypeEnum.SHOT_ON_TARGET,
    });
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Finalização no alvo do {teamLabel(showEventModal)}
                </h3>
                <button
                  onClick={() => setShowEventModal(null)}
              ...
              <form onSubmit={handleAddEvent} className="space-y-5">
                ...
                <div className="space-y-2">
                  <label htmlFor="scorerId" className="block text-sm font-medium text-foreground">
                    Autor da finalização no alvo <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="scorerId"
                    name="scorerId"
