...
const Nutmeg = ({ currentMatch, recordMatchEvents }: NutmegProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: "Erro",
        description: "Selecione o autor da caneta.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!opponentId) {
      toast({
        title: "Erro",
        description: "Selecione o jogador adversário (quem tomou a caneta).",
        variant: "destructive",
      });
      return;
    }
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Caneta do{" "}
              ...
              <form onSubmit={handleAddEvent} className="space-y-5">
                <div className="space-y-2">
                  ...
                  >
                    Autor da caneta <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="scorerId"
                    name="scorerId"
                ...
                <div className="space-y-2">
                  <label
                    ...
                    className="block text-sm font-medium text-foreground"
                  >
                    Quem tomou a caneta? <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="opponentId"
                    name="opponentId"
                  ...
                  >
                    <option value="">Selecione...</option>
                    {(showEventModal === 'teamA' ? teamPlayers('teamB') : teamPlayers('teamA')).map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name}
                      </option>
                    ))}
