...
type FreeKickScoredProps = {
  currentMatch: MatchViewModelType | null;
  recordMatchEvents: (events: RecordEventInputType[]) => Promise<void>;
}

const FreeKickScored = ({ currentMatch, recordMatchEvents }: FreeKickScoredProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: 'Erro',
        description: 'Selecione o autor do gol de falta.',
        variant: 'destructive',
      });
      return;
    }
    ...
    eventsToRecord.push({
      ...
      playerId: scorerId,
      statType: StatTypeEnum.FREE_KICK_SCORED,
    });
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Gol de falta do {teamLabel(showEventModal)}
                </h3>
                <button
                  onClick={() => setShowEventModal(null)}
