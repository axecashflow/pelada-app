...
const DuelWon = ({ currentMatch, recordMatchEvents }: DuelWonProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: 'Erro',
        description: 'Selecione o autor do duelo vencido.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!opponentId) {
      toast({
        title: 'Erro',
        description: 'Selecione quem perdeu o duelo.',
        variant: 'destructive',
      });
      return;
    }
    ...
    eventsToRecord.push({
      ...
      playerId: scorerId,
      statType: StatTypeEnum.DUEL_WON,
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Duelo Vencido do {teamLabel(showEventModal)}
                </h3>
                <button
                  onClick={() => setShowEventModal(null)}
              ...
              <form onSubmit={handleAddEvent} className="space-y-5">
                ...
                <div className="space-y-2">
                  <label htmlFor="scorerId" className="block text-sm font-medium text-foreground">
                    Vencedor do duelo <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="scorerId"
                    name="scorerId"
                ...
                <div className="space-y-2">
                  <label htmlFor="opponentId" className="block text-sm font-medium text-foreground">
                    Quem perdeu o duelo ? <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="opponentId"
                    name="opponentId"
