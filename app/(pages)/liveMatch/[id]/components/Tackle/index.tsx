...
const Tackle = ({ currentMatch, recordMatchEvents }: TackleProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: 'Erro',
        description: 'Selecione o autor do desarme.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!opponentId) {
      toast({
        title: 'Erro',
        description: 'Selecione quem perdeu a posse.',
        variant: 'destructive',
      });
      return;
    }
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Desarme do {teamLabel(showEventModal)}
                </h3>
                <button
                  onClick={() => setShowEventModal(null)}
              ...
              <form onSubmit={handleAddEvent} className="space-y-5">
                ...
                <div className="space-y-2">
                  <label htmlFor="scorerId" className="block text-sm font-medium text-foreground">
                    Autor do desarme <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="scorerId"
                    name="scorerId"
                ...
                <div className="space-y-2">
                  <label htmlFor="opponentId" className="block text-sm font-medium text-foreground">
                    Quem perdeu a posse? <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="opponentId"
                    name="opponentId"
