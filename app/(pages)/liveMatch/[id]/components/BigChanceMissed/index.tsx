...
type BigChanceMissedProps = {
  currentMatch: MatchViewModelType | null;
  recordMatchEvents: (events: RecordEventInputType[]) => Promise<void>;
}

const BigChanceMissed = ({ currentMatch, recordMatchEvents }: BigChanceMissedProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: 'Erro',
        description: 'Selecione o autor da grande chance perdida.',
        variant: 'destructive',
      });
      return;
    }
    ...
    eventsToRecord.push({
      ...
      playerId: scorerId,
      statType: StatTypeEnum.BIG_CHANCE_MISSED,
    });
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Grande chance perdida do {teamLabel(showEventModal)}
                </h3>
                <button
                  onClick={() => setShowEventModal(null)}
              ...
              <form onSubmit={handleAddEvent} className="space-y-5">
                ...
                <div className="space-y-2">
                  <label htmlFor="scorerId" className="block text-sm font-medium text-foreground">
                    Autor da grande chance perdida <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="scorerId"
                    name="scorerId"
                ...
                >
                  ...
                  Registrar grande chance perdida
                </SportButton>
              </form>
            </motion.div>
          </motion.div>
...
export default BigChanceMissed;
