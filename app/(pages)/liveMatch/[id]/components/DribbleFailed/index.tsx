...
const DribbleFailed = ({ currentMatch, recordMatchEvents }: DribbleFailedProps) => {
  ...
  const handleAddEvent: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!scorerId) {
      toast({
        title: 'Erro',
        description: 'Selecione o autor da falha de drible.',
        variant: 'destructive',
      });
      return;
    }
    ...
    eventsToRecord.push({
      ...
      playerId: scorerId,
      statType: StatTypeEnum.DRIBBLE_FAILED,
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">
                  {teamIcon(showEventModal)} Drible errado do {teamLabel(showEventModal)}
                </h3>
                <button
                  onClick={() => setShowEventModal(null)}
              ...
              <form onSubmit={handleAddEvent} className="space-y-5">
                ...
                <div className="space-y-2">
                  <label htmlFor="scorerId" className="block text-sm font-medium text-foreground">
                    Autor do drible errado <span className="text-destructive">*</span>
                  </label>
                  <select
                    id="scorerId"
                    name="scorerId"
                ...
                <div className="space-y-2">
                  <label htmlFor="opponentId" className="block text-sm font-medium text-foreground">
                    Quem recuperou a posse ? <span className="text-xs text-muted-foreground">(opcional)</span>
                  </label>
                  <select
                    id="opponentId"
                    name="opponentId"
